	...
	public void Apply( )
	{
		if( m_bIsDefined && m_rSerializedProperty != null )
		{
			...
			switch( m_rSerializedProperty.propertyType )
			{
				...
				{	
					// BugPatch : setting the property to the defaul value don't dirty this property even if the property has multiple value
					if(m_rSerializedProperty.hasMultipleDifferentValues)
					{
						// Set to an arbitrary different value before applying the new value
						m_rSerializedProperty.vector2Value = m_rSerializedProperty.vector2Value + Vector2.one;
					}
