...
public class Uni2DEditorSmoothBindingGUI
{
	...
	public static BoneEditMode CurrentBoneEditMode
	{
		...
		set
		{
			BoneEditMode ePreviousBoneEditMode = ms_eBoneEditMode;
			...
			if( value != BoneEditMode.None )
			{
				...
				else if( value == BoneEditMode.Posing )
				{
					...
					ms_rSprite.SetPosingMode(true);
				}

				switch( ePreviousBoneEditMode )
				{
					...
					{
						ms_rSprite.SetPosingMode(false);
						ms_rSprite.SaveSkeletonPosePosition();
			...
			else if( ePreviousBoneEditMode != BoneEditMode.None )	// Restore only if exiting edit mode
			{
				// Restore saved tool
				Tools.current  = ms_ePreviousTool;
				Tools.viewTool = ms_ePreviousViewTool;
				
				if( ePreviousBoneEditMode == BoneEditMode.Posing && ms_rSprite != null )
				{
					ms_rSprite.SetPosingMode(false);
					ms_rSprite.SaveSkeletonPosePosition();
	...
	public static bool CanUseAnimMode
	{
		get
		{
			return ms_rSprite != null && ms_rSprite.Bones.Count > 0;
		}
	}
	...
	public static bool CanUsePosingMode
	{
		get
		{
			return ms_rSprite != null && !MultiUnityVersionSupportUtility.InAnimationMode();
		}
	}
