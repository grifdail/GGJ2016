...
public class Uni2DSmoothBindingBone : MonoBehaviour
{
	[HideInInspector]
	[SerializeField]
	// Manipulated bone position
	private Uni2DBonePosition m_oBoneManipulatedPosition;
	
	[HideInInspector]
	[SerializeField]
	// Initial bone position
	private Uni2DBonePosition m_oBonePosePosition;
	
	// Current bone position
	private Uni2DBonePosition m_oBoneCurrentPosition;
	...
	public bool SetBonePosePosition()
	{
		if(m_oBonePosePosition == null || m_oBonePosePosition.IsInitialized == false)
		{
			return false;
		}
		else
		{
			m_oBonePosePosition.SetPosition(transform);
			return true;
		}
	}
	
	// Set bone manipulated position
	public void SetBoneManipulatedPosition()
	{
		if(m_oBoneManipulatedPosition != null && m_oBoneManipulatedPosition.IsInitialized)
		{
			m_oBoneManipulatedPosition.SetPosition(transform);
		}
	}
	
	// Offset
	public void OffsetBonePosition(Vector3 a_f3Offset)
	{
		if(m_oBoneManipulatedPosition != null)
		{
			m_oBoneManipulatedPosition.OffsetPosition(a_f3Offset);
			m_oBonePosePosition.OffsetPosition(a_f3Offset);
		}
	}
		...
		if( rSprite != null )
		{
			...
			if( oSpritePlane.Raycast( oWorldRay, out fDistance ) )
			{
				...
				if( rBoneToMoveParent != null )
				{
					...
					Vector3 f3CurrentDirection = rBoneToMoveParentTransform.InverseTransformPoint(transform.position);
					Vector3 f3NewDirection     = rBoneToMoveParentTransform.InverseTransformPoint(oWorldRay.GetPoint( fDistance ));
					...
					#endif
					rBoneToMoveParentTransform.localRotation *= oRot;
	...
	// Reset Bone Manipulated Position
	private void ResetBoneManipulatedPosition()
	{
		if(m_oBoneManipulatedPosition != null)
		{
			m_oBoneManipulatedPosition.CopyFrom(m_oBonePosePosition);
		}
	}
#endif
}