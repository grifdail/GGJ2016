...
public static class Uni2DEditorPolygonTriangulationUtils
{
	...
	public static Mesh PolygonizeContours( List<Contour> a_rDominantContoursList, Vector2 a_f2Scale, Vector3 a_f3PivotPoint, float a_fWidth, float a_fHeight )
	{
		// The mesh to build
		...
		List<Mesh> oSubMeshes = new List<Mesh>();
		for( int iContourIndex = 0; iContourIndex < iContourCount; ++iContourIndex )
		{
			...
			oSubMeshes.Add(oSubMesh);

			EarClipping( a_rDominantContoursList[ iContourIndex ], a_f2Scale, a_f3PivotPoint, a_fWidth, a_fHeight, out oVerticesArray, out oTrianglesArray, out oUVs );
		...
		// Clean
		foreach(Mesh rSubMesh in oSubMeshes)
		{
			if(rSubMesh != null)
			{
				Editor.DestroyImmediate(rSubMesh);
			}
		}
	...
	public static Mesh PolygonizeAndExtrudeContours( List<Contour> a_rDominantContoursList, float a_fExtrusionDepth, Vector2 a_f2Scale, Vector3 a_f3PivotPoint, bool a_bOnlyBorder )
	{
		// The mesh to build
		Mesh oCombinedMesh = new Mesh( );
		...
		for( int iContourIndex = 0; iContourIndex < iContourCount; ++iContourIndex )
		{
			...
			if(a_bOnlyBorder)
			{
				oSubMesh = PolygonizeContourBorder(rContour, a_fExtrusionDepth, a_f2Scale, a_f3PivotPoint);
			}
			else
			{
				oSubMesh = PolygonizeAndExtrudeContour(rContour, a_fExtrusionDepth, a_f2Scale, a_f3PivotPoint);
			}
	...
	public static List<Vector2> Get2DVerticesFromContours(List<Contour> a_rDominantContoursList, Vector2 a_f2Scale, Vector3 a_f3PivotPoint)
	{
	...
	private static void Add2DVerticesFromContour(Contour a_rContour, Vector2 a_f2Scale, Vector3 a_f3PivotPoint, ref List<Vector2> a_rVertices)
	{
		for(int i = 0; i < a_rContour.Count; ++i)
		{
			Vector3 f3Vertex = ((Vector3)a_rContour[i] - a_f3PivotPoint );
			f3Vertex.x *= a_f2Scale.x;
			f3Vertex.y *= a_f2Scale.y;
			a_rVertices.Add(f3Vertex);
		}
	}
	
	private static Mesh PolygonizeAndExtrudeContour(Contour a_rContour, float a_fExtrusionDepth, Vector2 a_f2Scale, Vector3 a_f3PivotPoint)
	{
		Vector3[ ] oVerticesArray;
		int[ ] oTrianglesArray;
		Vector2[ ] oUVs;
		
		EarClipping( a_rContour, a_f2Scale, a_f3PivotPoint, 1.0f, 1.0f, out oVerticesArray, out oTrianglesArray, out oUVs );
		Mesh oSubMesh = BuildExtrudedMeshFromPolygonizedContours( oVerticesArray, oTrianglesArray, a_fExtrusionDepth );
		
		return oSubMesh;
	}
	
	private static Mesh PolygonizeContourBorder(Contour a_rContour, float a_fExtrusionDepth, Vector2 a_f2Scale, Vector3 a_f3PivotPoint)
	{
		Vector3 f3ExtrusionOffset = a_fExtrusionDepth * Vector3.forward * 0.5f;
		
		List<Vector3> oVertices = new List<Vector3>();
		for(int i = 0; i < a_rContour.Count; ++i)
		{
			Vector3 f3SegmentBegin = ((Vector3)a_rContour[i] - a_f3PivotPoint );
			f3SegmentBegin.x *= a_f2Scale.x;
			f3SegmentBegin.y *= a_f2Scale.y;
			Vector3 f3SegmentEnd = ((Vector3)a_rContour[(i+1)%a_rContour.Count] - a_f3PivotPoint );
			f3SegmentEnd.x *= a_f2Scale.x;
			f3SegmentEnd.y *= a_f2Scale.y;
			
			Vector3 f3BeginFront = f3SegmentBegin - f3ExtrusionOffset;
			Vector3 f3EndFront = f3SegmentEnd - f3ExtrusionOffset;
			
			Vector3 f3BeginBack = f3SegmentBegin + f3ExtrusionOffset;
			Vector3 f3EndBack = f3SegmentEnd + f3ExtrusionOffset;
			
			// Triangle 1
			oVertices.Add(f3BeginFront);
			oVertices.Add(f3EndBack);
			oVertices.Add(f3EndFront);
			
			// Triangle 2
			oVertices.Add(f3BeginFront);
			oVertices.Add(f3BeginBack);
			oVertices.Add(f3EndBack);
		}
		
		int iVertexCount = oVertices.Count;
		
		int[] oIndices = new int[iVertexCount];
		for(int i = 0; i < iVertexCount; i++)
		{
			oIndices[i] = i;
		}
		
		Mesh oBorderMesh = new Mesh();
		
		oBorderMesh.vertices = oVertices.ToArray();
		oBorderMesh.triangles = oIndices;
		
		return oBorderMesh;
	}

	// Returns a list of extruded 3D triangles meshes from 2D outer/inner contours
	public static List<Mesh> CompoundPolygonizeAndExtrudeContours( List<Contour> a_rDominantContoursList, float a_fExtrusionDepth, Vector2 a_f2Scale, Vector3 a_f3PivotPoint )
	{
		...
		foreach( Contour rDominantContour in a_rDominantContoursList )
		{
			...
			EarClipping( rDominantContour, a_f2Scale, a_f3PivotPoint, 1.0f, 1.0f, out oVerticesArray, out oTrianglesArray, out oUVs );
