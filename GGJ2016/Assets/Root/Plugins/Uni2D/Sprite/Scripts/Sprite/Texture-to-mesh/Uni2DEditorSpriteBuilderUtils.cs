...
public static class Uni2DEditorSpriteBuilderUtils 
{
		...
		if( bRebuild )
		{
			...
			switch(a_rNewSpriteSettings.dimensionMode)
			{
				...
				case DimensionMode._2D:
				{
					...
					oMesh2D.vertices = PolygonizeTextureToBorder2DVerticesFromSettings(a_rNewSpriteSettings).ToArray();
					...
					if( a_rNewSpriteSettings.physicsMode != Uni2DSprite.PhysicsMode.NoPhysics )
					{
						...
						rPolygonCollider2D.points = oMesh2D.vertices;
					}
					#endif
				}
				break;
	...
	public static List<Vector2> PolygonizeTextureToBorder2DVerticesFromSettings( Uni2DEditorSpriteSettings a_rSpriteSettings )
	{
		if( a_rSpriteSettings.textureContainer == null || a_rSpriteSettings.physicsMode == Uni2DSprite.PhysicsMode.NoPhysics )
		{
		...
		List<Vector2> oVertices =  Uni2DEditorPolygonTriangulationUtils.Get2DVerticesFromContours(	oDominantContoursList, 
																									a_rSpriteSettings.ScaleFactor,
		                                                                                          	a_rSpriteSettings.PivotCoords);
	...
	{	
		...
		else
		{
			...
			#if AFTER_UNITY_4_3
			if(rSpriteData.polygonCollider2D != null)
			{
				Vector2[] oPoints = rSpriteData.polygonCollider2D.points;
				...
				rSpriteData.polygonCollider2D.points = oPoints;
				rSpriteData.polygonCollider2D.isTrigger = rSpriteSettings.isTrigger;
			}
			#endif
