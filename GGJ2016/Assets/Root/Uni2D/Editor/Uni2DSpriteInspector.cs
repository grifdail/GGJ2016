...
{
	#if BEFORE_UNITY_4_3
	private static string ms_GUIWarningMessage2DModeOnlyAfterUnity4_3  = "2D physics is only available from Unity 4.3 and later.";
	#endif
...
//	private static string ms_GUIButtonLabelCancel      = "Revert";
	...
	private const string mc_oGUILabelDimensionMode    = "Dimension";
	...
	private const string mc_oGUILabelOnlyBorder       = "Only Border";
	private const string mc_oGUILabelSubdivide        = "Subdivide";
	private const string mc_oGUILabelSubdivisionCount      = "Subdivision Count";
	...
	public override void OnInspectorGUI()
	{
		...
		bool bIsInAnimationMode = MultiUnityVersionSupportUtility.InAnimationMode();
		...
		EditorGUILayout.BeginVertical( );
		{
			...
			EditorGUI.BeginDisabledGroup( bNotEditable );
			{
				...
				bool bMixedSortingOrderLayerNameValue = false;
				string oSortingLayerName = "";
				bool bFirstSortingLayerNameValue = true;
				foreach(Uni2DSprite rSprite in targets )
				{
					string oCurrentSortingLayerNameValue = rSprite.SortingLayerName;
					if(bFirstSortingLayerNameValue == false && oCurrentSortingLayerNameValue != oSortingLayerName)
					{
						bMixedSortingOrderLayerNameValue = true;
						oSortingLayerName = "";
						break;
					}
					else
					{
						oSortingLayerName = oCurrentSortingLayerNameValue;
						bFirstSortingLayerNameValue = false;
					}
				}
				
				EditorGUI.BeginChangeCheck( );
				{
					EditorGUI.showMixedValue = bMixedSortingOrderLayerNameValue;
					{
						System.Reflection.PropertyInfo oPropertyInfo_SortingLayerNames = null;
						try 
						{
							System.Type oInternalEditorUtilityType = System.Type.GetType("UnityEditorInternal.InternalEditorUtility,UnityEditor");
							if (oInternalEditorUtilityType != null) 
							{
								oPropertyInfo_SortingLayerNames = oInternalEditorUtilityType.GetProperty("sortingLayerNames", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
							}
						}
						catch{}
						
						if(oPropertyInfo_SortingLayerNames != null) 
						{ 	
							string[] oSortingLayerNames = oPropertyInfo_SortingLayerNames.GetValue(null, null) as string[];
							
							int iSelectedItem = 0;
							string oLayerName = oSortingLayerName;
							if(oLayerName == "")
							{
								oLayerName = "Default";
							}
							for(int i = 0; i < oSortingLayerNames.Length; ++i)
							{
								if(oSortingLayerNames[i] == oLayerName)
								{
									iSelectedItem = i;
									break;
								}
							...
							oSortingLayerName = oSortingLayerNames[EditorGUILayout.Popup("Sorting Layer", iSelectedItem, oSortingLayerNames)];
						}
					}
					EditorGUI.showMixedValue = false;
				}
				if( EditorGUI.EndChangeCheck( ) )
				{
					foreach( Uni2DSprite rSprite in targets )
					{
						Undo.RecordObject(rSprite.GetComponent<Renderer>(), "Sorting Layer");
						rSprite.SortingLayerName = oSortingLayerName;
						EditorUtility.SetDirty(rSprite);
					}
				}
				
				///// Sorting Order /////
				
				bool bMixedSortingOrderValue = false;
				int iSortingOrder = 0;
				bool bFirstSortingOrderValue = true;
				foreach(Uni2DSprite rSprite in targets )
				{
					int iCurrentSortingOrderValue = rSprite.SortingOrder;
					if(bFirstSortingOrderValue == false && iCurrentSortingOrderValue != iSortingOrder)
					{
					...
					{
						iSortingOrder = iCurrentSortingOrderValue;
						bFirstSortingOrderValue = false;
					}
				}
				
				EditorGUI.BeginChangeCheck( );
				{
					EditorGUI.showMixedValue = bMixedSortingOrderValue;
					{
						iSortingOrder = EditorGUILayout.IntField( "Order in Layer", iSortingOrder);
					}
					EditorGUI.showMixedValue = false;
				}
				if( EditorGUI.EndChangeCheck( ) )
				{
					foreach( Uni2DSprite rSprite in targets )
					{
						Undo.RecordObject(rSprite.GetComponent<Renderer>(), "Order in Layer");
						rSprite.SortingOrder = iSortingOrder;
						EditorUtility.SetDirty(rSprite);
					}
				}
				#endif
				...
				SerializedSetting<SpriteScaleMode> rSerializedSpriteScaleMode    = m_oInteractiveBuildSettings.serializedSpriteScaleMode;
				SerializedSetting<float> rSerializedSpriteScale    = m_oInteractiveBuildSettings.serializedSpriteScale;
				SerializedSetting<Vector2> rSerializedSpriteScaleNotUniform    = m_oInteractiveBuildSettings.serializedSpriteScaleNotUniform;
				...
				EditorGUILayout.BeginVertical( );
				{
					EditorGUI.BeginChangeCheck();
					{
						Uni2DEditorGUIUtils.SerializedEnumPopup<SpriteScaleMode>(rSerializedSpriteScaleMode, "Sprite Scale Mode");
					}
					if(EditorGUI.EndChangeCheck())
					{
						for(int iTargetIndex = 0; iTargetIndex < iTargetCount; ++iTargetIndex)
						{
							Uni2DEditorSpriteSettings rSpriteSettings = ((Uni2DSprite) targets[iTargetIndex]).SpriteSettings;
							if(rSpriteSettings.spriteScaleMode != rSerializedSpriteScaleMode.Value)
							{
								switch(rSerializedSpriteScaleMode.Value)
								{
									case SpriteScaleMode.NotUniform:
									{
										rSpriteSettings.spriteScaleNotUniform = rSpriteSettings.spriteScale * Vector2.one;
									}
									break;
									
									case SpriteScaleMode.Uniform:
									{
										rSpriteSettings.spriteScale = rSpriteSettings.spriteScaleNotUniform.x;
									}
									break;
								}
							}
					...
					EditorGUILayout.BeginHorizontal( );
					{
						if(rSerializedSpriteScaleMode.HasMultipleDifferentValues == false)
						{
							switch(rSerializedSpriteScaleMode.Value)
							{
								case SpriteScaleMode.NotUniform:
								{
									EditorGUI.BeginChangeCheck( );
									{
										SerializedVector2Field(rSerializedSpriteScaleNotUniform, mc_oGUILabelScale);
									}
									if(EditorGUI.EndChangeCheck())
									{
										rSerializedSpriteScale.Value = rSerializedSpriteScaleNotUniform.Value.x;
									}
								}
								break;
								...
								case SpriteScaleMode.Uniform:
								{
									...
									if(EditorGUI.EndChangeCheck())
									{
										rSerializedSpriteScaleNotUniform.Value = rSerializedSpriteScale.Value * Vector2.one;
									}
								}
								break;
							}
					...
					// Pixel perfect
					EditorGUI.BeginDisabledGroup(rSerializedSpriteScale.HasMultipleDifferentValues == false && rSerializedSpriteScaleNotUniform.HasMultipleDifferentValues == false
						 && rSerializedSpriteScale.Value == 1.0f && rSerializedSpriteScaleNotUniform.Value == Vector2.one);
					{
						if(GUILayout.Button( "Pixel Perfect", GUILayout.Width(mc_GUISmallButtonWidth)))
						{
							rSerializedSpriteScale.Value = 1.0f;
							rSerializedSpriteScaleNotUniform.Value = Vector2.one;
						}
					}
					EditorGUI.EndDisabledGroup( );
					...
					if( ms_bGUIFoldoutPhysicMode )
					{
						++EditorGUI.indentLevel;
						{
							...
							{
								// Is Trigger
								SerializedToggle(m_oInteractiveBuildSettings.serializedIsTrigger, "Is Trigger");
								...
								// DimensionMode Mode
								Uni2DEditorGUIUtils.SerializedEnumPopup<DimensionMode>(m_oBuildSettings.serializedDimensionMode, mc_oGUILabelDimensionMode);
								
								bool bUnavailablePhysicMode = false;
								#if BEFORE_UNITY_4_3
								bool b2DSelected = m_oBuildSettings.serializedDimensionMode.HasMultipleDifferentValues ||  m_oBuildSettings.serializedDimensionMode.Value == DimensionMode._2D;
								if(b2DSelected)
								{
									bUnavailablePhysicMode = true;
									EditorGUILayout.HelpBox(ms_GUIWarningMessage2DModeOnlyAfterUnity4_3, MessageType.Warning, true);
								}
								#endif
								
								EditorGUI.BeginDisabledGroup(bUnavailablePhysicMode);
								{
									
									bool bDisable3DSettings = m_oBuildSettings.serializedDimensionMode.HasMultipleDifferentValues ||  m_oBuildSettings.serializedDimensionMode.Value == DimensionMode._2D;
									
									EditorGUI.BeginDisabledGroup( bDisable3DSettings );
									{
										// Collision type
									...
									if( ms_bGUIFoldoutColliderSettings )
									{
										++EditorGUI.indentLevel;
										{
											...
											// Subdivide
											SerializedToggle( m_oBuildSettings.serializedSubdivide, mc_oGUILabelSubdivide );
																				
											// Subdivision Step
											bool bDisableSubdivisionStep = m_oBuildSettings.serializedSubdivide.HasMultipleDifferentValues
												|| m_oBuildSettings.serializedSubdivide.Value == false;
											
											EditorGUI.BeginDisabledGroup( bDisableSubdivisionStep );
											{
												EditorGUI.BeginChangeCheck( );
												{
													SerializedSlider( m_oBuildSettings.serializedSubdivisionCount, Uni2DEditorSpriteSettings.SubdivisionCountMin, Uni2DEditorSpriteSettings.SubdivisionCountMax, mc_oGUILabelSubdivisionCount );
												}
												if( EditorGUI.EndChangeCheck( ) )
												{
													this.StopRegenTimer( );
											...
											bool bDisablePolygonizeHoles = rSerializedCollisionType.HasMultipleDifferentValues
												|| (rSerializedCollisionType.Value == Uni2DSprite.CollisionType.Convex && m_oBuildSettings.serializedDimensionMode.HasMultipleDifferentValues == false &&  m_oBuildSettings.serializedDimensionMode.Value == DimensionMode._3D);
											...
											EditorGUI.BeginDisabledGroup(bDisable3DSettings);
											{
												...
												// Only border
												bool bDisableOnlyBorder = rSerializedCollisionType.HasMultipleDifferentValues
													|| rSerializedCollisionType.Value == Uni2DSprite.CollisionType.Compound || rSerializedCollisionType.Value == Uni2DSprite.CollisionType.Convex;
												
												EditorGUI.BeginDisabledGroup( bDisableOnlyBorder );
												{
													SerializedToggle( m_oBuildSettings.serializedOnlyBorder, mc_oGUILabelOnlyBorder );
												}
												EditorGUI.EndDisabledGroup();
											}
											EditorGUI.EndDisabledGroup();
				...
				{
					rSerializedSpriteScale.Value = ClampScale(rSerializedSpriteScale.Value);
				}
				
				if( rSerializedSpriteScaleNotUniform.IsDefined )
				{
					rSerializedSpriteScaleNotUniform.Value = ClampScale(rSerializedSpriteScaleNotUniform.Value);
				}
			...
			if( ms_bGUIFoldoutSkeletalSmoothBinding )
			{
				++EditorGUI.indentLevel;
				{
					...
					SerializedSetting<Uni2DSprite.EPhysicsSkinMode> rSerializedPhysicsSkinMode    = m_oInteractiveBuildSettings.serializedPhysicsSkinMode;
					SerializedSetting<Uni2DSprite.ERenderSkinMode> rSerializedRenderSkinMode    = m_oInteractiveBuildSettings.serializedRenderSkinMode;
					...
					EditorGUI.BeginDisabledGroup( bNotEditable );
					{
						...
						Uni2DEditorGUIUtils.SerializedEnumPopup<Uni2DSprite.EPhysicsSkinMode>( rSerializedPhysicsSkinMode, "Physics Skin Mode" );
						Uni2DEditorGUIUtils.SerializedEnumPopup<Uni2DSprite.ERenderSkinMode>( rSerializedRenderSkinMode, "Render Skin Mode" );
	...
	*/
	...
	private float ClampScale(float a_fScale)
	{
		return Mathf.Clamp(Mathf.Abs(a_fScale), Uni2DEditorSpriteSettings.ScaleMin, float.MaxValue) * Mathf.Sign(a_fScale);
	}
	
	private Vector2 ClampScale(Vector2 a_f2Scale)
	{
		a_f2Scale.x = ClampScale(a_f2Scale.x);
		a_f2Scale.y = ClampScale(a_f2Scale.y);
